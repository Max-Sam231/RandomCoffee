    ...
    <div style={{display: 'flex',
      ...
      <div className="StatisticContainer">
        ...
        <div>
          ...
          <span>Понедельник<Progress percent={Math.round((weekday[1]/dataMeet.length)*100) }  showInfo={true} format={(percent) => `${weekday[1]}`}/></span>
          <span>Вторник<Progress percent={Math.round((weekday[2]/dataMeet.length)*100) }  showInfo={true} format={(percent) => `${weekday[2]}`}/></span>
          <span>Среда<Progress percent={Math.round((weekday[3]/dataMeet.length)*100) }  showInfo={true} format={(percent) => `${weekday[3]}`}/></span>
          <span>Четверг<Progress percent={Math.round((weekday[4]/dataMeet.length)*100) }  showInfo={true} format={(percent) => `${weekday[4]}`}/></span>
          <span>Пятница<Progress percent={Math.round((weekday[5]/dataMeet.length)*100) }  showInfo={true} format={(percent) => `${weekday[5]}`}/></span>
          <span>Суббота<Progress percent={Math.round((weekday[6]/dataMeet.length)*100) }  showInfo={true} format={(percent) => `${weekday[6]}`}/></span>
          <span>Воскресенье<Progress percent={Math.round((weekday[0]/dataMeet.length)*100) }  showInfo={true} format={(percent) => `${weekday[0]}`}/></span>
        </div>
      }
      </div>
      <div className="StatisticContainer">
